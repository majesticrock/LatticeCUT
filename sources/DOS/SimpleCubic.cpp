...
SimpleCubic::SimpleCubic(std::size_t N, double E_F, double debye) : Base(N, -1, 1, E_F, factor_range * debye) {}
...
inline _internal_precision I_1(const abscissa_t& epsilon) {
    ...
    _internal_precision ret = static_cast<_internal_precision>(asin(upper_bound) * R(upper_bound + epsilon) -
                                                               asin(lower_bound) * R(lower_bound + epsilon));

    auto integrand = [&epsilon](const abscissa_t& phi) { return asin(phi) * derivative_R(phi + epsilon); };
...
void SimpleCubic::compute() {
    ...
    for (int k = 1; k < std::ssize(_dos) - 1; ++k) {
        ...
        try {
            _dos[k] = ratio * _energies.get_dE(k) *
                      static_cast<double>(boost::math::pow<3>(LONG_1_PI) * (I_1(epsilon) - I_2(epsilon)));
