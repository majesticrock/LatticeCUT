...
FreeElectrons::FreeElectrons(std::size_t N, double band_width, int _dimension, double E_F, double debye)
...
double FreeElectrons::normalization(double mass /* =1 */, double hbar /* =1 */) const {
    return std::pow(2 * mass / (hbar * hbar), 0.5 * dimension) /
           (std::tgamma(0.5 * dimension) * std::pow(2 * _pi, dimension));
}

void FreeElectrons::compute() {
    const double An = normalization();
    for (std::size_t i = 0; i < _dos.size(); i++) {
