...
FCC::FCC(std::size_t N, double E_F, double debye) : Base(N, -1. + 1e-10, 1., E_F, factor_range * debye) {}

dos_complex k_minus(dos_complex const& one_over_z) {
    return one_half - 2.L * one_over_z * std::pow(1.L + one_over_z, -three_halves) -
           one_half * (1.L - one_over_z) * std::pow(1.L + one_over_z, -three_halves) *
               std::sqrt(1.L - 3.L * one_over_z);
}
...
void FCC::compute() {
    ...
    for (int k = 0; k < std::ssize(_dos); ++k) {
        ...
        const dos_complex ellint = one_half * LONG_PI *
                                   mrock::utility::Numerics::hypergeometric_2F1(
                                       one_half, one_half, _internal_precision{1}, k_minus(one_over_z));
