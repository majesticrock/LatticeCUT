...
DOS::EnergyRanges::EnergyRanges(double _E_min,
                                double _E_max,
                                [[maybe_unused]] double _inner_min,
                                [[maybe_unused]] double _inner_max,
                                int _N)
    : E_min{_E_min},
      E_max{_E_max},
      total_range{E_max - E_min},
#ifndef UNIFORM_DISCRETIZATION
      inner_min{std::max(E_min, _inner_min)},
      inner_max{std::min(E_max, _inner_max)},
...
int DOS::EnergyRanges::energy_to_index(double epsilon) const {
    ...
    return static_cast<int>(std::lround((epsilon - inner_max) / upper_dE) + inner_discretization +
                            lower_discretization);
#endif
}
