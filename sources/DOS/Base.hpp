...
_CONST_LONG_FLOATING FOUR_OVER_PI_SQR = boost::math::constants::two_div_pi<_internal_precision>() *
                                        boost::math::constants::two_div_pi<_internal_precision>();  // 4 / pi^2
...
struct Base {
    ...
    inline EnergyRanges const& get_energies() const { return this->_energies; }

    void normalize();

    inline double get_min_energy() const noexcept { return _energies.E_min; }
    inline double get_max_energy() const noexcept { return _energies.E_min; }
    inline std::size_t size() const noexcept { return _dos.size(); }
