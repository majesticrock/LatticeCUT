...
#pragma omp parallel for
    ...
    if (this->local_interaction == l_float{}) {
        // The order parameter must have the same complex phase everywhere, but we may not touch the chemical potential,
        // hence we skip the last element
...
#pragma omp parallel for reduction(+ : u_contribution)
        for (int l = 0; l < N; ++l) {
            u_contribution += _expecs[mrock::symbolic_operators::OperatorType::SC][l >= loop_bound ? N - 1 - l : l] *
                              density_of_states[l];
        }
        for (int k = 0; k < loop_bound; k++) {
...
l_float DOSModel::compute_filling(const l_float mu) const {
    auto number_operator = [&](int i) -> l_float {
        ...
        const l_float E = std::sqrt(eps * eps + Delta[i] * Delta[i]);
        if (beta < 0)
            return 0.5 * (1 - (eps / E));
        else
...
l_float DOSModel::compute_coefficient(mrock::symbolic_operators::Coefficient const& coeff,
                                      int first,
                                      int second) const {
...
const std::map<mrock::symbolic_operators::OperatorType, std::vector<l_float>>& DOSModel::get_expectation_values()
    const {
    if (_expecs.empty()) {
...
l_float DOSModel::delta_max() const {
    return std::abs(*std::max_element(Delta.begin(), Delta.begin() + N, [](const l_float& lhs, const l_float& rhs) {
...
std::string DOSModel::info() const {
    return "DOSModel: g=" + std::to_string(phonon_coupling) + "\tU=" + std::to_string(local_interaction_energy_units) +
           "\tDOS=" + dos_name;
}
