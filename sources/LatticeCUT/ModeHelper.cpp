...
#define COMMUTATOR_DIR "../commutators/lattice_cut/"
#endif
    ...
    if (term.sums.momenta.empty()) {
        ...
        if (!term.coefficients.empty()) {
            ...
            if (coeff_ptr->momenta.size() == 2U) {
                value *= model->compute_coefficient(*coeff_ptr, select_epsilon(coeff_ptr->momenta[0], k, l),
                                                    select_epsilon(coeff_ptr->momenta[1], k, l));
...
std::vector<l_float> ModeHelper::continuum_boundaries() const {
    ...
    std::cout << "Found minimum at epsilon=" << min << "   E(min)=" << prev_min << "   E(0)="
              << model->quasiparticle_energy_index(model->energies.energy_to_index(model->chemical_potential))
              << std::endl;
    return {2 * prev_min, 2 * std::max(model->quasiparticle_energy_index(loop_offset),
                                       model->quasiparticle_energy_index(loop_end - 1))};
}
...
#else
    auto solver = mrock::utility::Selfconsistency::make_broyden<l_float>(model.get(), &model->Delta,
                                                                         model->local_interaction < 0.3 ? 100 : 0);
#endif
