...
namespace LatticeCUT {
    ComplexModel::ComplexModel(mrock::utility::InputFileReader& input) 
        ...
        Delta(decltype(Delta)::FromAllocator([&](int k) -> std::complex<l_float> {
            if (k == N) {
                return fermi_energy;
            }
    ...
    void ComplexModel::iteration_step(const ParameterVector& initial_values, ParameterVector& result)
    {
        ...
        if (!guaranteed_E_F) this->chemical_potential = std::real(initial_values(N));
...
#pragma omp parallel for
    ...
    l_float ComplexModel::compute_filling(const l_float mu) const
    {
        auto number_operator = [&](int i) -> l_float {
            ...
            const l_float E = sqrt(eps*eps + std::norm(Delta[i]));
            if (is_zero(Delta[i]))
    ...
    l_float ComplexModel::compute_coefficient(mrock::symbolic_operators::Coefficient const &coeff, int first, int second) const
    {
    ...
    const std::map<mrock::symbolic_operators::OperatorType, std::vector<std::complex<l_float>>> &ComplexModel::get_expectation_values() const
    {
		if (_expecs.empty()) {
...
#pragma omp parallel for
    ...
    {
        return "ComplexModel: g=" + std::to_string(phonon_coupling) + "\tU=" + std::to_string(local_interaction_energy_units) + "\tDOS=" + dos_name;
    }
