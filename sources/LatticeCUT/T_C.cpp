...
T_C::T_C(mrock::utility::InputFileReader& input) : model(DOSModel(input)) {
...
void T_C::compute() {
    ...
    for (std::size_t i = 0U; i < max_gaps.size(); ++i) {
        max_gaps[i] = std::abs(std::ranges::max(
...
std::string T_C::to_folder() const {
    ...
    return "T_C/" + model.dos_name + "/N=" + std::to_string(model.N) + "/" +
           "g=" + improved_string(model.phonon_coupling_in) + "/" + "U=" + improved_string(model.local_interaction) +
