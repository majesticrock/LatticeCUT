...
namespace LatticeCUT {
    struct ComplexModel {
        typedef Eigen::VectorXcd ParameterVector;
        ...
        ModelAttributes<std::complex<l_float>> Delta;

        ComplexModel(mrock::utility::InputFileReader& input);

        void iteration_step(const ParameterVector& initial_values, ParameterVector& result);
        
        std::complex<l_float> sc_expectation_value_index(int k) const;
        ...
        const std::map<mrock::symbolic_operators::OperatorType, std::vector<std::complex<l_float>>>& get_expectation_values() const;
    ...
    private:
		mutable std::map<mrock::symbolic_operators::OperatorType, std::vector<std::complex<l_float>>> _expecs;
