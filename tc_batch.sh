...
for NEW_VALUE in "${NEW_VALUES[@]}"; do
  ...
  while read line; do
    ...
    if [[ "$TOKEN_NAME" == "$TOKEN" ]]; then
      sed "s/$TOKEN_NAME $TOKEN_VALUE/$TOKEN_NAME $NEW_VALUE/" params/tc.config > auto_generated_${CURRENT_TIME}/$NEW_NAME.config
      break
    fi
  done < params/tc.config

  # Create the slurm file with modifications
  slurm_path="auto_generated_${CURRENT_TIME}/$NEW_NAME.slurm"
  sed -e "s|#SBATCH --job-name=T_c|#SBATCH --job-name=T_c_${NEW_NAME}_${CURRENT_TIME}|" \
      -e "s|#SBATCH --output=/home/althueser/phd/cpp/LatticeCUT/output_T_c.txt|#SBATCH --output=/home/althueser/phd/cpp/LatticeCUT/output_${CURRENT_TIME}_$NEW_NAME.txt|" \
